...
bool get_current_processes(){
    cprocesses.clear();
    if(next_pid>=processes.size())
        return false;
    if(cprocesses.empty()){
        time_elapsed = processes[next_pid]->AT;
...
class RR:public Scheduler{
            ...
            void reduce_dynamic_priority(process* proc){
                if(proc->dprio==0)
...
void simulate_processes(Scheduler* scheduler){
    ...
    while((e = get_event())){
        ...
        put_event(&proc,proc->last_state,RUNNG,time_elapsed);
        }
    }
}
...
void print_process_timeline(process* proc){
    time_elapsed = max(proc->AT,time_elapsed);
    cout<<time_elapsed<<" "<<proc->PID<<" "<<0
        <<": "<<"CREATED"<<" -> "<<"READY"<<endl;
    int last_burst=0;
    while(proc->TC>0){  // while process is still not done
        int cb =get_new_burst(proc->CB);
        if(cb>=proc->TC){
            cb = proc->TC;
            proc->TC = 0;
        }
        ...
        cout<<time_elapsed<<" "<<proc->PID<<" "<<0
        <<": "<<"READY"<<" -> "<<"RUNNG"
        <<" "<<"cb="<<cb<<" rem="<<proc->TC+cb
        <<" prio="<<proc->dprio<<endl;
        time_elapsed+=cb;
        if(proc->TC==0)
            break;
        int io = get_new_burst(proc->IO);
        cout<<time_elapsed<<" "<<proc->PID<<" "<<cb
        <<": "<<"RUNNG"<<" -> "<<"BLOCK "
        <<" "<<"ib="<<io<<" rem="<<proc->TC<<endl;
        time_elapsed+=io;

        cout<<time_elapsed<<" "<<proc->PID<<" "<<io
        <<": "<<"BLOCK"<<" -> "<<"READY"<<endl;
    }
    cout<<time_elapsed<<" "<<proc->PID<<" "<<last_burst
        <<": "<<"Done"<<endl;
}
